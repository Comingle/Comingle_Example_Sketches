...
bool clicked = false;
CapacitiveSensor   cs_Sender_Sensor = CapacitiveSensor(A5,A4);
...
void setup() {
  ...
  Toy.attachLongPressStart(longPress);
   ...
   for (int i = 0; i < 100; i++) {
     long totalc =  cs_Sender_Sensor.capacitiveSensorRaw(30);
    calibration += totalc;
    delay(20);
  }
  calibration=calibration/100;
  
  //Blip again after calibration
    startupSequence();
    startupSequence();
...
void loop() {
 ...
 /* long total3 =  cs_Sender_Sensor.capacitiveSensor(30);
   ...
   ,0,255);
*/
long total3 =  cs_Sender_Sensor.capacitiveSensorRaw(30);
long diff= abs( total3-calibration);

    Serial.println(diff);                // print sensor output 3
    	
    Toy.setOutput(0, diff);
        Toy.setOutput(1, diff-100);
            Toy.setOutput(2, diff+100);
}
