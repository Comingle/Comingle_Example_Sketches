 ...
 * eg: ./muse-io --device 00:06:66:69:4F:8B --50hz --dsp --osc "osc.udp://localhost:5000"
 //this one
 muse-io  --60hz --dsp --osc "osc.udp://localhost:5000" --preset 10 
 //preset doesn't work
...
//We are going to to choose 3 at a time, one for each motor 
...
//Here's  all the paths for the Muse
...
//Looking at binary details like blinking and clenching

      if (theOscMessage.checkTypetag("i")) {
        // For each of the channels of data (4, in preset 10)
        for (int i=0; i<numChannels; i++) {
          // Store the current data point in the array
          values[curWave][i] = theOscMessage.get(i).intValue();
